...
class DPS(nn.Module):
       ...
       def forward(self,logits,mux_out):
              ...
              if self.training == True:
                  noise = self.gumbel.sample((self.batch_size,self.mux_in,1)).repeat(1,1,mux_out).to(self.device)
              else:
                  noise = 0
              ...
              batch_indices = torch.tensor([range(self.batch_size)]).reshape(self.batch_size,1).repeat(1,mux_out)
              ...
              masking_matrix = torch.zeros(self.batch_size,self.mux_in,mux_out).to(self.device)
