...
class DPSMask(nn.Module):
    ...
    def forward(self, x):
           ...
           return output_image.unsqueeze(4),mask
...
class PGADPSMask(nn.Module):
    def __init__(self,args):
           ...
           if self.mux_out == 0: # Set minimum mux_out to 1 if the int(args.bands*args.percentage_As/100) <1 
                  self.mux_out = 1
