...
class Network(nn.Module):
    def __init__(self,args):
           ...
           if self.sampling == 'DPS':
                  self.no_iter = 1
                  self.logit_parameter = nn.Parameter(torch.randn(1024)/4.)
                  ...
                  self.logit_parameter = nn.Parameter(torch.randn(1024)/4.)
                  self.mux_out_first = int(args.no_pixels*args.percentage_DPS/100)
                  self.mux_out = int(args.no_pixels*args.percentage_Block_ADPS/100)
    ...
    def forward(self, x):
           ...
           x_vector = x.reshape(batch_size,3,1024)
           ...
           for i in range(self.no_iter):
                  if self.Ref:
                     ...
                     y = mask.unsqueeze(1)*x_vector
                     y = y.reshape(batch_size,3,32,32)
