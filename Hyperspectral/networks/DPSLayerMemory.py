...
class DPS(nn.Module):
        ...
        def forward(self,logits,mux_out):
                # %% forward call              
                #create perturbed logits
                batch_size=logits.shape[0]
                if self.training == True:
                    noise = self.gumbel.sample((batch_size,self.mux_in,1)).repeat(1,1,mux_out).to(self.device)
                else:
                    noise = 0
                
                perturbed_logits = logits.unsqueeze(2).repeat(1,1,mux_out) + noise
                
                #masking from memory
                masking_matrix_memory = self.sample_memory.unsqueeze(2).repeat(1,1,mux_out) * -1000
                perturbed_logits += masking_matrix_memory
                
                #find the locations of the topk samples
                _,topk_indices = torch.topk(perturbed_logits[:,:,0],mux_out)
                
                # %% counting vectors (helpers)
                batch_indices = torch.tensor([range(batch_size)]).reshape(batch_size,1).repeat(1,mux_out)
                k_counter = torch.tensor([range(mux_out)])

                # %% create  hard sample mask
                hard_sample_mask = torch.zeros(batch_size,self.mux_in).to(self.device)
                ...
                masking_matrix = torch.zeros(batch_size,self.mux_in,mux_out).to(self.device)
        ...
        def initialize_sample_memory(self,input_image):
                batch_size = input_image.shape[0]
                self.sample_memory = torch.zeros(batch_size,self.mux_in).to(self.device)